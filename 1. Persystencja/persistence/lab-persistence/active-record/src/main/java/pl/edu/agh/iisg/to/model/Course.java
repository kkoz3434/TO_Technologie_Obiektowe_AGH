...
public class Course {
    ...
    public static Optional<Course> findById(final int id) {
    	...
    	try {
	        ...
	        return Optional.of(new Course(rs.getInt("id"),
                    rs.getString("name")));
		} catch (SQLException e) {
			e.printStackTrace();
		}
    ...
    public boolean enrollStudent(final Student student) {
        // TODO jesli nie student nie zapisany: zapisz i true, else false
        String enrollStudentSql = "INSERT INTO student_course VALUES(?, ?)";
        Object[] args = {student.id(), this.id()};

        if(enrollStudentSql.contains(student.))
        //TODO
